...
def calculate_rtp_and_roi(initial_rtp, withdraw_amounts, roi_rates, support_rate, months, delay_months):
    """
    Reklam harcaması, ROI ve devlet katkısı hesaplama fonksiyonu.

    Args:
        initial_rtp (float): İlk ay reklam harcaması miktarı.
    ...
    next_rtp_investments = [0] * num_months  # Bir sonraki ay reklam harcaması miktarları
    total_investments = [0] * num_months  # Toplam reklam harcaması miktarları
    support_included = ["Hayır"] * num_months  # Destek dahil mi?
    profit = [0] * num_months  # Hacim
    ...
    for i in range(1, num_months):
        # ROI hesapla (önceki ayın reklam harcaması üzerinden)
        ...
        # Bir sonraki ay reklam harcaması miktarını belirle
        if i < delay_months:
    ...
    data = {
        "Aylar": months,
        "Aylık Reklam Harcaması Miktarı (₺)": total_investments,
        "ROI Oranı": roi_rates,
...
def main():
    ...
    st.title("Dezztech Return System Reklam Harcaması Desteği Hesaplama Modülü")
    st.write("Bu araç, reklam harcaması, ROI ve devlet katkısını hesaplar.")

    # Kullanıcıdan giriş alın
    initial_rtp = st.number_input("İlk Ay Reklam Harcaması Miktarı (₺)", min_value=0.0, value=100000.0, step=1000.0, format="%.2f")
    ...
    if st.button("Hesapla"):
        ...
        total_investment = results["Aylık Reklam Harcaması Miktarı (₺)"].sum()
        total_profit = results["Hacim (₺)"].sum()
        ...
        st.write(f"Toplam Hacim: {total_profit:,.2f} ₺")
        ...
        ax[2].plot(results["Aylar"], results["Hacim (₺)"], marker='o', color='green')
        ax[2].set_title("Aylık Hacim")
        ax[2].set_xlabel("Aylar")
        ax[2].set_ylabel("Hacim (₺)")
